    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><p class="introtext">REQUISIÇÃO DE HORAS </p>
                ...
                <h4 class="modal-title">Data : <?php echo $dia.'/'.$mes.'<br>'; ?></h4>
            </div>
                ...
                ?>
                    ...
                    <div class="form-group">
                        <?= lang('Descrição da atividade', 'title'); ?>
                            <?php echo form_input('title', (isset($_POST['title']) ? $_POST['title'] : $data_periodo->descricao), 'maxlength="200" class="form-control input-tip" required="required" id="title"'); ?>
                    </div>
                    <div class="form-group">
                        <?= lang("Selecione a ação vinculada a requisição", "slVinculoAcao"); ?>
                        <?php //if($data_periodo->tipo_registro){ ?>
                        <div id="credito" style="display:<?php if($data_periodo->tipo_registro == 'Crédito'){ ?> block <?php }else{ ?>  none <?php } ?>;">
                            <?php                                       

                            $usuario_sessao = $this->session->userdata('user_id'); 
                            $acoes = $this->atas_model->getAllPlanosUser($usuario_sessao);
                            //$wu_acao['N/A'] = 'N/A (Não está relacionado a uma ação) ';
                            ...
                            echo form_dropdown('acoes_vinculo', $wu_acao, (isset($_POST['acoes_vinculo']) ? $_POST['acoes_vinculo'] : $data_periodo->id_acao), 'id="slVinculoAcao"  class="form-control  select" required="required" data-placeholder="' . lang("Selecione a(s) Ações(es)") . ' "   style="width:100%;" ');
                            ?>
                        </div>   
                        
                        <div id="debito" style="display:<?php if($data_periodo->tipo_registro == 'Débito'){ ?> block <?php }else{ ?>  none <?php } ?>;">
                            <?php                                       
                            $wu_acao2['N/A'] = 'N/A (Débito)';
                            
                            echo form_dropdown('acoes_vinculo', $wu_acao2, (isset($_POST['acoes_vinculo']) ? $_POST['acoes_vinculo'] : ""), 'id="slVinculoAcao"  class="form-control  select" required="required" data-placeholder="' . lang("Selecione a(s) Ações(es)") . ' "   style="width:100%;" ');
                            ?>
                        </div> 
                    ...
                    <div class="row">
                        <div class="col-md-3">
                            <div class="form-group">
                                <?= lang('start', 'start'); ?>
                                <input type="time" required="true" value="<?php echo $data_periodo->hora_inicio; ?>" class="form-control" name="start">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                <?= lang('Fim', 'end'); ?>
                              <input type="time" required="true" value="<?php echo $data_periodo->hora_fim_confirmado; ?>" class="form-control" name="end">
                              </div>
                        </div>
                         <div class="col-md-3">
                            <div class="form-group">
                                <?= lang('Tipo de Hora', 'tipo'); ?>
                                      ...
                                      echo form_dropdown('tipo', $pst, (isset($_POST['tipo']) ? $_POST['tipo'] : $data_periodo->tipo_registro), 'id="tipo" onchange="optionCheck()"  class="form-control " data-placeholder="' . lang("select") . ' ' . lang("") . '" required="required"   style="width:100%;" ');
                                     ?>
